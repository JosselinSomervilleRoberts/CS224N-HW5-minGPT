...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Step 5: Encode the resulting input and output strings as Long tensors and return the resulting data point
        input = torch.LongTensor([self.stoi[c] for c in input])
        output = torch.LongTensor([self.stoi[c] for c in output])
